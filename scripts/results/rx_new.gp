...
set label "(DetnetFlow)" at 0.8,-0.7
set label "(DetnetFlow)" at 1.6,-0.7
set label "(DetnetFlow)" at 4.6,-0.7

#set arrow 2 from 4.5,3 to 4.5,3.5
#set label "Bw Reservation\nPer Detnet flow" at 4,4

plot \
'<cat rx_bwresv_mid.dat'	u ($2):($4)/1000 t 'Rx Bwresv' axes x1y1 with linespoints lw 2 lc rgb "blue" pt fc ps 1,\
'<cat rx_fd_mid.dat'		u ($2):($4)/1000 t 'Rx FD' axes x1y1 with linespoints lw 2 pt fs lc rgb "dark-green",\
'<cat rx_fifo_mid.dat'	u ($2):($4)/1000 t 'Rx FIFO' axes x1y1 with linespoints lw 1.5 pt et lc rgb "brown",\
'<cat rx_ared_mid.dat'	u ($2):($4)/1000 t 'Rx ARED' axes x1y1 with linespoints lw 1.5 pt ed lc rgb "dark-violet",\
'<cat rx_stresv_mid.dat'	u ($2):($4)/1000 t 'Rx STRESV' axes x1y1 with linespoints lw 1.5 pt fi lc rgb "black",\
'<cat tx_stresv_mid.dat'	u ($2):($4)/1000 t 'Tx' axes x1y1 with linespoints lw 2 lc rgb "red" pt fs
